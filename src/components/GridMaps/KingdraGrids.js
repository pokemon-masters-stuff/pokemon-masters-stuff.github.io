...
import { kingdraGridData } from './PokemonData/SyncGridData';
import {
  addToGridList,
  removeFromGridList,
...
class KingdraGrids extends Component {
  constructor(props) {
    super(props);

    this.state = {
      isSelected: kingdraGridData.map(element => false)
    };
  ...
  handleClickReset() {
    this.setState({ isSelected: kingdraGridData.map(element => false) });
    this.props.resetGrids();
  }

  render() {
    const allGrids = kingdraGridData.map((cell, index) => {
      if (index == 0) {
        return (
          <Hexagon
            key={index}
