...
import { serperiorGridData } from './PokemonData/SyncGridData';
import {
  addToGridList,
  removeFromGridList,
...
class SerperiorGrids extends Component {
  constructor(props) {
    super(props);

    this.state = {
      isSelected: serperiorGridData.map(element => false)
    };
  ...
  handleClickReset() {
    this.setState({ isSelected: serperiorGridData.map(element => false) });
    this.props.resetGrids();
  }

  render() {
    const allGrids = serperiorGridData.map((cell, index) => {
      if (index === 0) {
        return (
          <Hexagon
            key={index}
