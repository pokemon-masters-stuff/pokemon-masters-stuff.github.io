...
import { infernapeGridData } from './PokemonData/SyncGridData';
import {
  addToGridList,
  removeFromGridList,
...
class InfernapeGrids extends Component {
  constructor(props) {
    super(props);

    this.state = {
      isSelected: infernapeGridData.map(element => false)
    };
  ...
  handleClickReset() {
    this.setState({ isSelected: infernapeGridData.map(element => false) });
    this.props.resetGrids();
  }

  render() {
    const allGrids = infernapeGridData.map((cell, index) => {
      if (index == 0) {
        return (
          <Hexagon
            key={index}
